...
export default function TotalFlag({ flagId, total }: PriceFlagProps) {
   const flag = useMemo(() => {
   ...
   return (
      ...
      >
         ...
         {flag?.color !== 'transparent' && (
            <div className='sm:opacity-0 sm:group-hover:opacity-100 btn-circle h-5 w-5 p-1 absolute top-0 right-0 bg-red-600'>
               <TrashIcon />
            </div>
         )}
      </button>
